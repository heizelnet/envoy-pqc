...
void DefaultCertValidator::updateDigestForSessionId(bssl::ScopedEVP_MD_CTX& md,
  ...
  int rc;
  ENVOY_LOG_MISC(info, "[+]DefaultCertValidator::updateDigestForSessionId start");
  ...
  // is enabled.
  if (ca_cert_ != nullptr) {
    ENVOY_LOG_MISC(info, "[+]DefaultCertValidator::updateDigestForSessionId X509_digest");
    rc = X509_digest(ca_cert_.get(), EVP_sha256(), hash_buffer, &hash_length);
    RELEASE_ASSERT(rc == 1, Utility::getLastCryptoError().value_or(""));
    RELEASE_ASSERT(hash_length == SHA256_DIGEST_LENGTH,
                   fmt::format("invalid SHA256 hash length {}", hash_length));

    ENVOY_LOG_MISC(info, "[+]DefaultCertValidator::updateDigestForSessionId EVP_DigestUpdate");
