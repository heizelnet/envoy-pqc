...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config,
  ...
  for (size_t i = 0; i < tls_contexts_.size(); i++) {
    ...
    ENVOY_LOG_MISC(info, "[+]ContextImpl::ContextImpl - {}", "SSL_CTX_new");
    ...
    ENVOY_LOG_MISC(info, "[+]ContextImpl::ContextImpl cipher suites: {}", config.cipherSuites().c_str());
    ...
    ENVOY_LOG_MISC(info, "[+]ContextImpl::ContextImpl - {}", "SSL_CTX_set1_curves_list");
    if (!capabilities_.provides_ciphers_and_curves &&
      ...
      ENVOY_LOG_MISC(info, "[+]ContextImpl::ContextImpl - Failed to initialize ECDH curves: {}", config.ecdhCurves().c_str());
      return;
    }
  ...
  auto verify_mode_or_error = cert_validator_->initializeSslContexts(ssl_contexts, config.capabilities().provides_certificates);
...
// #endif
  ...
  stat_name_set_->rememberBuiltins({"P-256", "P-384", "P-521", "X25519", "X25519Kyber768Draft00", "X25519MLKEM768"});
...
ContextImpl::newSsl(const Network::TransportSocketOptionsConstSharedPtr& options,
  ...
  ENVOY_LOG_MISC(info, "[+]ContextImpl::newSsl - {}", "SSL_new()");
...
enum ssl_verify_result_t ContextImpl::customVerifyCallback(SSL* ssl, uint8_t* out_alert) {
  ENVOY_LOG_MISC(info, "[+]ContextImpl::customVerifyCallback - {}", "SSL_get_ex_data()");
...
absl::Status TlsContext::loadCertificateChain(const std::string& data,
  ...
  ENVOY_LOG_MISC(info, "[+]TlsContext::loadCertificateChain!- {}", "BIO_new_mem_buf");
