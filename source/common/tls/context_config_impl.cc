...
ContextConfigImpl::ContextConfigImpl(
  ...
  ENVOY_LOG_MISC(info, "[+]ContextConfigImpl::ContextConfigImpl() - ecdh_curves in config: {}", config.tls_params().ecdh_curves());
  ENVOY_LOG_MISC(info, "[+]ContextConfigImpl::ContextConfigImpl() - "
                       "min_protocol_version: {}, max_protocol_version: {}, cipher_suites: {}, "
                       "ecdh_curves: {}, signature_algorithms: {}, alpn_protocols: {}",
                 min_protocol_version_, max_protocol_version_, cipher_suites_, ecdh_curves_,
                 signature_algorithms_, alpn_protocols_);
...
ClientContextConfigImpl::ClientContextConfigImpl(
    ...
    : ContextConfigImpl(config.common_tls_context(), config.auto_sni_san_validation(),
                        ...
                        "X25519MLKEM768", factory_context, creation_status),
